...
function Calculator() {
  ...
  const calculateResults = ({ amount, interest, years }) => {
    ...
    if (isFinite(monthly)) {
      const monthlyPaymentCalculated = `${monthly.toFixed(2) + " Rupees"}`;
      const totalPaymentCalculated = `${
        (monthly * calculatedPayments).toFixed(2) + " Rupees"
      }`;
      const totalInterestCalculated = `${
        (monthly * calculatedPayments - userAmount).toFixed(2) + " Rupees"
      }`;
